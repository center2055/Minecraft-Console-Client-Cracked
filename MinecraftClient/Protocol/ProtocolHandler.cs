...
namespace MinecraftClient.Protocol
{
    ...
    public static class ProtocolHandler
    {
        ...
        public static LoginResult GetLogin(string user, string pass, LoginType type, out SessionToken session)
        {
            ...
            else if (type == LoginType.offline)
            {
                return OfflineLogin(user, pass, out session);
            }
            else
                throw new InvalidOperationException(
                    "Account type must be Mojang, Microsoft, Offline, or valid authlib 3rd Servers!");
        }

        private static LoginResult OfflineLogin(string user, string pass, out SessionToken session)
        {
            session = new SessionToken
            {
                ID = string.Empty,
                PlayerName = user,
                PlayerID = GenerateOfflineUuid(user),
        ...
        private static string GenerateOfflineUuid(string username)
        {
            ...
            byte[] data = md5.ComputeHash(Encoding.UTF8.GetBytes("OfflinePlayer:" + username));
            data[6] &= 0x0F;
            data[6] |= 0x30;
            data[8] &= 0x3F;
            data[8] |= 0x80;
