...
namespace MinecraftClient
{
    ...
    public class McClient : IMinecraftComHandler
    {
        ...
        public void OnGameJoined(bool isOnlineMode)
        {
            ...
            if (Config.Main.General.AccountType == LoginType.offline)
            {
                var offlinePassword = InternalConfig.Account.Password;
                if (!string.IsNullOrWhiteSpace(offlinePassword) && offlinePassword != "-")
                {
                    SendText($"/login {offlinePassword}");
                }
                else
                {
                    Log.Warn("Offline password not provided; skipped /login command.");
                }
            }
